...
contract TicketMarketplace is ReentrancyGuard, Ownable, MarketPlaceErrorsEnumsAndStruct {
    ...
    /*//////////////////////////////////////////////////////////////
                                EVENTS
    //////////////////////////////////////////////////////////////*/
    event FundsDeposited(address indexed user, address indexed eventContract, uint256 amount, string depositType);
    event FundsWithdrawn(address indexed user, address indexed eventContract, uint256 amount);
    event PrimarySaleRegistered(address indexed user, address indexed eventContract, uint256 amount);
    event Listed(
        address indexed seller, address indexed tokenContract, uint256 indexed tokenId, uint256 price, SaleType saleType
    );
    event AuctionCreated(bytes32 indexed listingId, uint256 startTime, uint256 endTime, uint256 reservePrice);
    event BidPlaced(bytes32 indexed listingId, address indexed bidder, uint256 amount, uint256 timestamp);
    event AuctionSettled(bytes32 indexed listingId, address indexed winner, uint256 finalPrice);
    event ItemSold(address indexed buyer, address indexed seller, bytes32 indexed listingId, uint256 price);
    ...
    event EmergencyRefundClaimed(address indexed user, address indexed eventContract, uint256 amount);
    event ProfitsCollected(address indexed user, address indexed eventContract, uint256 amount);
    event EventEnded(address indexed eventContract);
    event RoyaltyPaid(address indexed eventContract, address indexed recipient, uint256 amount);
    ...
    event EventContractAuthorized(address indexed eventContract, bool authorized);
    ...
    modifier onlyVerifiedUser() {
        require(userVerification.isVerifiedAndActive(msg.sender), "Marketplace: User not verified or is inactive");
        _;
    }
    ...
    /*//////////////////////////////////////////////////////////////
                                CONSTRUCTOR
    //////////////////////////////////////////////////////////////*/
    constructor(address _platformAddress, uint256 _platformFeePercent, address _userVerfierAddress)
        Ownable(msg.sender)
     ...
     */
    function depositForEvent(address eventContract) external payable nonReentrant notPaused onlyVerifiedUser {
     ...
     */
    function registerPrimarySale(address user, address organizer, uint256 organizerPercentage)
        external
        payable
        nonReentrant
        onlyAuthorizedEventContract
    {
        ...
        uint256 organizerShare = (totalAmount * organizerPercentage) / BASIS_POINTS;
        uint256 platformShare = totalAmount - organizerShare;

        if (organizerShare > 0) {
            userBalances[organizer][eventContract].lockedBalance += organizerShare;
        }
        if (platformShare > 0) {
     ...
     */
    function _registerDeposit(address user, address eventContract, uint256 amount, string memory depositType)
        internal
    {
     ...
     */
    function withdrawFunds(address eventContract, uint256 amount) external nonReentrant onlyVerifiedUser {
     ...
     */
    function authorizeEventContract(address eventContract, bool authorized) external {
    ...
    function listItemFixedPrice(address tokenContract, uint256 tokenId, uint256 price)
        external
        notPaused
        onlyVerifiedUser
    {
        ...
        emit Listed(msg.sender, tokenContract, tokenId, price, SaleType.FIXED_PRICE);
    }
    ...
    function createAuction(
        ...
        emit Listed(msg.sender, tokenContract, tokenId, startingPrice, SaleType.AUCTION);
        emit AuctionCreated(listingId, auction.startTime, auction.endTime, reservePrice);
    }
     ...
     */
    function buyItemWithDeposits(address tokenContract, uint256 tokenId)
        external
        nonReentrant
        notPaused
        onlyVerifiedUser
    {
        ...
        UserBalance storage buyerBalance = userBalances[msg.sender][tokenContract];
        if (buyerBalance.availableBalance < listing.price) {
            revert Marketplace__InsufficientDeposits(listing.price, buyerBalance.availableBalance);
        }
        // Execute the purchase using internal balances
        buyerBalance.availableBalance -= listing.price;
        _distributeSaleProceeds(tokenContract, listing.tokenId, listing.price, listing.seller);
     ...
     */
    function placeBidWithDeposits(address tokenContract, uint256 tokenId, uint256 bidAmount)
        external
        nonReentrant
        notPaused
        onlyVerifiedUser
    {
        ...
        UserBalance storage bidderBalance = userBalances[msg.sender][tokenContract];
        if (bidderBalance.availableBalance < bidAmount) {
            revert Marketplace__InsufficientDeposits(bidAmount, bidderBalance.availableBalance);
        }
        ...
        if (auction.highestBidder != address(0) && auction.highestBidder != msg.sender) {
            UserBalance storage prevBidderBalance = userBalances[auction.highestBidder][tokenContract];
        ...
        if (auction.endTime - block.timestamp < 600 && auction.extensionCount < MAX_AUCTION_EXTENSIONS) {
            auction.endTime += 600;
     ...
     * @dev Settle auction
     */
    function settleAuction(address tokenContract, uint256 tokenId) external nonReentrant notPaused {
        bytes32 listingId = getListingId(tokenContract, tokenId);
        Listing storage listing = listings[listingId];
        Auction storage auction = auctions[listingId];
        ...
        IERC721(tokenContract).transferFrom(listing.seller, auction.highestBidder, tokenId);

        // Settle funds from the winner's locked bid
        UserBalance storage winnerBalance = userBalances[auction.highestBidder][tokenContract];
        winnerBalance.lockedBalance -= auction.highestBid; // The bid is now spent
        _distributeSaleProceeds(tokenContract, tokenId, auction.highestBid, listing.seller);
        ...
        for (uint256 i = 0; i < auction.bidders.length; i++) {
            ...
            if (bidder != auction.highestBidder) {
                ...
                if (bidAmount > 0) {
                    UserBalance storage bidderBalance = userBalances[bidder][tokenContract];
                    bidderBalance.availableBalance += bidAmount;
                    bidderBalance.lockedBalance -= bidAmount;
                }
            }
     ...
     */
    function collectProfits(address eventContract) external nonReentrant onlyVerifiedUser {
        if (!eventInfo[eventContract].eventEnded) {
    ...
    function claimEmergencyRefund(address eventContract) external nonReentrant {
        ...
        uint256 refundAmount = info.userOriginalDeposits[msg.sender] + balance.lockedBalance;
     ...
     */
    function withdrawRoyalties(address eventContract, address payable recipient) external nonReentrant {
        if (!eventInfo[eventContract].eventEnded) {
            revert Marketplace__EventNotEnded();
        }
    ...
    function _distributeSaleProceeds(address tokenContract, uint256 tokenId, uint256 price, address seller) internal {
        // 1. Calculate Royalty
        uint256 royaltyAmount = 0;
        address royaltyRecipient = address(0);

        if (IERC165(tokenContract).supportsInterface(type(IERC2981).interfaceId)) {
            try IERC2981(tokenContract).royaltyInfo(tokenId, price) returns (address recipient, uint256 rAmount) {
    ...
    function _validatePriceIncrease(bytes32 listingId, uint256 newPrice, address tokenContract) internal view {
    ...
    function _refundAllBidders(bytes32 listingId, address tokenContract) internal {
        Auction storage auction = auctions[listingId];
    ...
    function _trackMarketplaceUsage(address tokenContract, address user, uint256 tokenId) internal {
        (bool success,) =
            tokenContract.call(abi.encodeWithSignature("trackMarketplaceUsage(address,uint256)", user, tokenId));
        // Ignore failure
    }

    function _getEventCancellationStatus(address tokenContract) internal view returns (bool) {
        (bool success, bytes memory data) = tokenContract.staticcall(abi.encodeWithSignature("eventCancelled()"));
    ...
    function _getMintPrice(address tokenContract) internal view returns (uint256) {
        (bool success, bytes memory data) = tokenContract.staticcall(abi.encodeWithSignature("baseMintPrice()"));
        ...
        )
    {
        UserBalance storage balance = userBalances[user][eventContract];
        uint256 maxWithdraw =
            balance.totalDeposited > balance.totalWithdrawn ? balance.totalDeposited - balance.totalWithdrawn : 0;
        ...
        return (
            ...
            maxWithdraw > balance.availableBalance ? balance.availableBalance : maxWithdraw
        );
    }
    ...
    function getUserOriginalDeposit(address user, address eventContract) external view returns (uint256) {
        return eventInfo[eventContract].userOriginalDeposits[user];
    }

    function getRoyaltiesPayable(address eventContract, address recipient) external view returns (uint256) {
        return royaltiesPayable[eventContract][recipient];
    }

    function getPlatformFeesPayable(address eventContract) external view returns (uint256) {
